...
export function LeistungsnachweisDialog({ open, onClose, onSubmit, defaultValues, klientenstammdatenList, enablePhotoScan = false }: LeistungsnachweisDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await onSubmit(fields as Leistungsnachweis['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{defaultValues ? 'Leistungsnachweis bearbeiten' : 'Leistungsnachweis hinzufügen'}</DialogTitle>
        </DialogHeader>
